                ...
                .execute(new StringCallback() {
                    ...
                    public void onSuccess(String resultStr, Call call, Response response) {
                        try {
                            ...
                            // data是Bean
                            if (httpResult.data != null && !"".equals(httpResult.data.toString())) {
                                if (isList) {
                                    httpResult.data = JSON.parseArray(httpResult.data.toString(), dataClass);
                                } else {
                                    httpResult.data = JSON.parseObject(httpResult.data.toString(), dataClass);
                                }
                            }
                            ...
                            if (callback != null) {
                                callback.upComplete(HttpResult.getExceptionResult(HttpResult.ERROR_DATA, resultStr));
                            }
                        }
                    }
