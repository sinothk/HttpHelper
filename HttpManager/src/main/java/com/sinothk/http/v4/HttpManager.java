...
//                            @Override
//                            public void run() {
                        httpCallback.onComplete(HttpResult.getExceptionResult(HttpResult.ERROR_LOCAL, e.getMessage()));
//                            }
//                        });
                    }
                });
     ...
     * @param callback
     */
    public static void post(String url, HttpParams httpParams, final HttpCallback callback) {
        if (httpParams == null) {
...
//                .upString("这是要上传的长文本数据！")//
                ...
                .execute(new StringCallback() {
                    ...
                    public void onError(Call call, Response response, final Exception e) {
                        if (callback != null) {
                            callback.onComplete(HttpResult.getExceptionResult(HttpResult.ERROR_LOCAL, e.getMessage().toString()));
                        }
                    }
                });
     ...
     * @param callback
     */
    public static void post(String url, HttpParams httpParams, final Class<?> dataClass, final boolean isList, final HttpCallback callback) {
        if (httpParams == null) {
            httpParams = new HttpParams();
        }
                ...
                .execute(new StringCallback() {
                    ...
                    public void onSuccess(String resultStr, Call call, Response response) {
                        try {
                            ...
                            // data是Bean
                            if (httpResult.serviceContent != null && !"".equals(httpResult.serviceContent.toString())) {
                                if (isList) {
                                    httpResult.serviceContent = JSON.parseArray(httpResult.serviceContent.toString(), dataClass);
                                } else {
                                    httpResult.serviceContent = JSON.parseObject(httpResult.serviceContent.toString(), dataClass);
                                }
                            }
...
//    }
     ...
     * @param callback
     */
    public static void postHttpParamsAndFile(String url, HttpParams httpParams, File file, final HttpProgressCallback callback) {
        OkGo.post(url)//
                .tag(url)//
...
//                .addFileParams("key", List<File> files)	// 这里支持一个key传多个文件
                ...
                .execute(new StringCallback() {
                    ...
                    public void onError(Call call, Response response, Exception e) {
                        if (callback != null) {
                            callback.onComplete(HttpResult.getExceptionResult(HttpResult.ERROR_LOCAL, e.getMessage()));
                        }
                        e.printStackTrace();
                    }
                });
     ...
     * @param callback
     */
    public static void postHttpParamsAndFile(String url, HttpParams httpParams, File file, final Class<?> dataClass, final boolean isList, final HttpProgressCallback callback) {
        OkGo.post(url)//
                .tag(url)//
                .isMultipart(true)       // 强制使用 multipart/form-data 表单上传（只是演示，不需要的话不要设置。默认就是false）
//                .params("param1", "paramValue1") 		// 这里可以上传参数
...
//                .addFileParams("key", List<File> files)	// 这里支持一个key传多个文件
     ...
     * @param callback
     */
    public static void postHttpParamsAndFileList(String url, HttpParams httpParams, List<File> fileList, final Class<?> dataClass, final boolean isList, final HttpProgressCallback callback) {
        OkGo.post(url)//
                .tag(url)//
                .isMultipart(true)       // 强制使用 multipart/form-data 表单上传（只是演示，不需要的话不要设置。默认就是false）
//                .params("param1", "paramValue1") 		// 这里可以上传参数
